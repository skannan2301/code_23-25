...
% Define paths
saveERPPath = '/Volumes/T7/flamingo/p300s'; % folder where your ERP files are located 
...
for k = 1:length(erpList)
    try
        ...
        [maxAmplitude, idx] = max(ERP.bindata(ch, timeIndices));        
        latencyAtMax = ERP.times(timeIndices(idx));
        ...
        T = table({ID}, maxAmplitude, latencyAtMax, ...
          'VariableNames', {'subject_id_info', 'maxAmplitude', 'latencyAtMax'});
...
writetable(masterTable, '/Volumes/T7/flamingo/p300_amp_lat_output.csv');
